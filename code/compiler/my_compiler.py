...
def compile_array_literal(node: ArrayLiteral, emitter: Emitter):
    elements = []
    elem_type = compiler_type(node=node.type.subtype, emitter=emitter)
    ...
    array_type = f"[{node.size} x {elem_type}]"
...
def get_llvm_function(operation: str, type: Type):
    if operation == "+":
        return "fadd" if isinstance(type, FloatType) else "add"
    elif operation == "-":
        return "fsub" if isinstance(type, FloatType) else "sub"
    elif operation == "*":
        return "fmul" if isinstance(type, FloatType) else "mul"
    elif operation == "/":
        return "fdiv" if isinstance(type, FloatType) else "sdiv"
    elif operation == "%":
        return "frem" if isinstance(type, FloatType) else "srem"
    elif operation == "<":
        return "fcmp olt" if isinstance(type, FloatType) else "icmp slt"
    elif operation == ">":
        return "fcmp ogt" if isinstance(type, FloatType) else "icmp sgt"
    elif operation == "<=":
        return "fcmp ole" if isinstance(type, FloatType) else "icmp sle"
    elif operation == ">=":
        return "fcmp oge" if isinstance(type, FloatType) else "icmp sge"
    elif operation == "=":
        return "fcmp oeq" if isinstance(type, FloatType) else "icmp eq"
    elif operation == "!=":
        return "fcmp one" if isinstance(type, FloatType) else "icmp ne"
    elif operation == "&&":
        return "and"
    elif operation == "||":
        return "or"
